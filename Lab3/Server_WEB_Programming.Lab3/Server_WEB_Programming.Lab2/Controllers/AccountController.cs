            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    using (var httpClient = new HttpClient())
                    {
                        ...
                        HttpResponseMessage response = httpClient.PostAsync("https://localhost:44355/Token", encodedRequest).Result;

                        var res = await response.Content.ReadAsStringAsync();

                        token = JsonConvert.DeserializeObject<BearerToken>(res);
